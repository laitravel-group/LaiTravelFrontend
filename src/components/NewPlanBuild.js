...
export default function NewPlanBuild(props) {
  ...
  for (let i = 0; i < numDays; i++) {
    items1.push({
      ...
      label: (
        <span>
          <CalendarOutlined /> {[dates[0].add(i, "day").format("MMM Do")]}
        </span>
      ),
    });
  }
  ...
  for (let i = 0; i < numDays; ++i) {
    data.push({
      ...
      date: (
        <span>
          <CalendarOutlined /> {[dates[0].add(i, "day").format("dddd, MMM Do")]}
        </span>
      ),
    });
  }
              ...
              <Dropdown
                menu={{
                  ...
                  onClick: (e) => {
                    ...
                    moving.time = dates[0].hour(8).minute(0);
                    moving.duration = dates[0].hour(3).minute(0);
                    moving.marker = cloneDeep(markers[record.key]);
                    newPlans[e.key].push(moving);
                    for (let i = 0; i < newPlans[e.key].length; i++) {
                      newPlans[e.key][i].key = i.toString();
  ...
  const findMultiLocation = () => {
    if (mapApiLoaded) {
      ...
      const request = {
        location: myPosition,
        radius: 30000,
        keyword: searchKeyword === "" ? "famous travel spot" : searchKeyword,
