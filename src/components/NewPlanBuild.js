...
export default function NewPlanBuild(props) {
	const { cityLocation, dates, placesData } = props;
	...
	for (i = 0; i < placesData.length; i++) {
		recommendations[i] = placesData[i];
		recommendations[i].key = i.toString();
	}
	...
	const dataSearch = [
		{
			...
			place: (
				<span>
					<SearchOutlined /> Recommendations
				</span>
			),
		},
		{
