  ...
  return (
    <Layout style={{ height: "100vh" }}>
      {proposedPlans.length > 0 ? (
        <div>
          <h3>Date: {dayjs(proposedPlans[0].date).format("YYYY-MM-DD")}</h3>
          <p>Start Location: {proposedPlans[0].startLocation.placeName}</p>
          <p>Start Time: {dayjs(proposedPlans[0].startTime).format("HH:mm")}</p>
          <h4>Visits:</h4>
          ...
          <button onClick={() => setTripPlanOnCurrentDay(proposedPlans[0])}>
            Apply
          </button>
        </div>
      ) : (
