...
export default function PlaceVisitEdit({ tripPlan, setTripPlan, currentDay }) {
	...
	const showDeleteConfirm = (placeIndex) => {
		confirm({
			...
			onOk() {
				...
				newTripPlan.details[currentDay].visits.splice(placeIndex, 1);
	...
	const movePlace = (placeIndex, selectedDay) => {
		const newTripPlan = { ...tripPlan };
		const moving = newTripPlan.details[currentDay].visits[placeIndex];
		newTripPlan.details[selectedDay].visits.push(moving);
		newTripPlan.details[currentDay].visits.splice(placeIndex, 1);
		setTripPlan(newTripPlan);
	};
	...
	const setStayDuration = (stayDuration, placeVisitIndex) => {
		...
		if (stayDuration === null) {
			newTripPlan.details[currentDay].visits[
				placeVisitIndex
			].stayDuration = null;
			setTripPlan(newTripPlan);
			return;
		}
		newTripPlan.details[currentDay].visits[placeVisitIndex].stayDuration =
